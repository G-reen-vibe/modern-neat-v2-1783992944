"""
MORPH v14 - Tuned version with soft L0 + aggressive restart.

Round 15: Based on ablation findings:
- L0 pressure too aggressive (default 0.02 hurts Acrobot). Use soft L0 (0.005, 0.05).
- Diverse init helps MountainCar but slightly hurts Acrobot. Keep it (MountainCar
  is harder, exploration matters more).
- Fitness shaping helps reliability. Keep.
- Restart helps but needs to be more aggressive for MountainCar. Reduce stagnation
  limit to 8 (was 15).

This is the "tuned v12" - the version we'll use for final comparisons.
"""
...
class MorphV14:
    """Tuned MORPH: soft L0 + aggressive restart + diverse init + fitness shaping."""

    def __init__(self, n_inputs, n_outputs, n_hidden_max=16,
                 pop_size=30, sigma0=1.5,
                 l0_pressure=0.005, l0_threshold=0.05,  # SOFT L0
                 init_gate_logit_on=1.0, init_gate_logit_off=-1.0,
                 init_topology_diversity=0.3,
                 stagnation_limit=8,  # MORE AGGRESSIVE restart
                 sigma_restart_threshold=1e-3,  # higher threshold
                 max_restarts=5,
                 pop_doubling=True,
                 fitness_shaping_weight=0.15,  # slightly stronger
